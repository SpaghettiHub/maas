...
The landline telephone system slowly evolved into our modern networks.  [Network switching](https://en.wikipedia.org/wiki/Network_switch#firstHeading)**^** grew out of [crossbar](https://en.wikipedia.org/wiki/Number_Five_Crossbar_Switching_System#firstHeading)**^**, which routed local phone calls to the correct subscriber.  Every number dialled closed one more relay, with all seven relays making a complete circuit between telephones.
...
These [framesets](https://en.wikipedia.org/wiki/Distribution_frame#firstHeading)**^** replaced switchboard operators; no more delays finding the right plug and connecting your call (1287) to the plug for the called party (3274).  Over time, these relay networks got much simpler by allowing exchanges to "swallow" dialled digits. 
