...
class RackAgent(RackOnlyExternalService):
    ...
    def _getConfiguration(self) -> agent_config.Configuration:
        ...
        return agent_config.Configuration(
            ...
            temporal=agent_config.TemporalConfiguration(
                host=urlparse(controllers[0]).hostname,
                port=5271,
                encryption_key=MAAS_SHARED_SECRET.get(),
            ),
            services=agent_config.Services(
                http_proxy=agent_config.HTTPProxyConfiguration(
                    cache=agent_config.HTTPProxyCache(
            ...
            observability=agent_config.Observability(
                logging=agent_config.LogConfiguration(
                    "debug" if debug_enabled else "info",
                ),
            ),
            tls=agent_config.TLS(
