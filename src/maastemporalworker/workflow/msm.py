...
from maascommon.enums.node import NodeStatusEnum, NodeTypeEnum
    ...
    async def get_heartbeat_data(self) -> MachineStatsByStatus:
        ...
        async with self.start_transaction() as tx:
            ...
            for row in result.all():
                match row.status:
                    case NodeStatusEnum.ALLOCATED:
                        ret.allocated += row.total
                    case NodeStatusEnum.DEPLOYED:
                        ret.deployed += row.total
                    case NodeStatusEnum.READY:
                        ret.ready += row.total
                    case (
                        NodeStatusEnum.FAILED_COMMISSIONING
                        | NodeStatusEnum.FAILED_DEPLOYMENT
                        | NodeStatusEnum.FAILED_DISK_ERASING
                        | NodeStatusEnum.FAILED_ENTERING_RESCUE_MODE
                        | NodeStatusEnum.FAILED_EXITING_RESCUE_MODE
                        | NodeStatusEnum.FAILED_RELEASING
                        | NodeStatusEnum.FAILED_TESTING
                    ):
                        ret.error += row.total
                    case _:
                        ret.other += row.total
