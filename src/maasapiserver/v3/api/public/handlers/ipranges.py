...
class IPRangesHandler(Handler):
    ...
    async def create_fabric_vlan_subnet_iprange(
        ...
        vlan_id: int,
        subnet_id: int,
        iprange_request: IPRangeRequest,
        response: Response,
        services: ServiceCollectionV3 = Depends(services),
            ...
            raise BadRequestException(
                details=[
                    BaseExceptionDetail(
                        ...
                        message="Only the admin can create ip ranges on behalf of other users.",
                    )
                ]
            )
    ...
    async def update_fabric_vlan_subnet_iprange(
        ...
        if (
            ...
            raise BadRequestException(
                details=[
                    BaseExceptionDetail(
                        type=INVALID_ARGUMENT_VIOLATION_TYPE,
                        message="Only the admin can update ip ranges owned by other users.",
                    )
                ]
            )
        subnet = await services.subnets.get_one(
        ...
        # populate the owner_id of the request to make checks in the to_builder method.
        if iprange_request.owner_id is None:
            if authenticated_user.is_admin():
                iprange_request.owner_id = iprange.user_id
            else:
