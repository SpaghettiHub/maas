...
class DjangoSessionAuthenticationProvider(AuthenticationProvider):
    async def authenticate(
        ...
        return AuthenticatedUser(
            ...
            # TODO: Candid async client still to be implemented. In order to quickly enable the UI to integrate with the first
            #  v3 endpoints, let's consider everybody a simple user.
            roles={UserRole.USER},
        )
...
class V3AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def _session_authentication(self, request: Request, sessionid: str):
        session_provider = self.providers_cache.get_session_provider()
        return await session_provider.authenticate(request, sessionid)

    async def _jwt_authentication(self, request: Request, auth_header: str):
