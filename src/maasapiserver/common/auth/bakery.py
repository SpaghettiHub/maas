...
from macaroonbakery.httpbakery._interactor import (
    LegacyInteractor,
...
async def discharge_all(m, get_discharge, local_key=None):
...
class HttpBakeryAsyncClient:
    ...
    async def acquire_discharge(self, cav, payload) -> bakery.Macaroon:
        ...
        if resp.status == 200 and resp_json != None:
    ...
    async def _legacy_interact(self, location, error_info) -> bakery.Macaroon:
        ...
        if (
            len(self._interaction_methods) > 1
            or self._interaction_methods[0].kind()
            != WEB_BROWSER_INTERACTION_KIND
        ):
