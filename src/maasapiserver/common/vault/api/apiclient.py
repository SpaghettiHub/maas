...
class AsyncVaultApiClient:
    def __init__(
        self,
        base_url: str,
        # TODO: https://warthogs.atlassian.net/browse/MAASENG-3344 add proxy support
    ):
    ...
    async def kv_v2_read(
        ...
        """
        async with self._client_session() as client:
            print(f"/v1/{kv_v2_mount_path}/data/{path}")
            response = await client.get(
    ...
    async def _client_session(
        ...
        async with ClientSession(base_url=self._base_url) as client:
            yield client
