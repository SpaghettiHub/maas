...
class RepositoryCommonTests(abc.ABC, Generic[T]):
    ...
    @pytest.fixture
    @abc.abstractmethod
    async def _instance_builder(self, *args, **kwargs) -> ResourceBuilder:
    ...
    async def test_create(
        self,
        repository_instance: BaseRepository,
        _instance_builder: ResourceBuilder,
    ):
        now = utcnow()
        resource = (
            _instance_builder.with_created(now).with_updated(now).build()
        )
    ...
    async def test_delete(
        ...
        await repository_instance.delete_by_id(_created_instance.id)
        deleted = await repository_instance.get_by_id(_created_instance.id)
    ...
    async def test_update(
        ...
        now = utcnow()
        builder = _instance_builder.with_created(now).with_updated(now)
