...
class TestIPRangesRepository(RepositoryCommonTests[IPRange]):
    ...
    @pytest.fixture
    async def _created_instance(self, fixture: Fixture) -> IPRange:
        subnet = await create_test_subnet_entry(
            fixture, name="name", description="description"
        )
        return IPRange(
    ...
    @pytest.fixture
    async def _instance_builder(self) -> IPRangeResourceBuilder:
        return (
            IPRangeResourceBuilder()
            .with_type("type")
            .with_start_ip(IPv4Address("10.0.0.1"))
    ...
    async def test_create_duplicated(
        ...
        _instance_builder: IPRangeResourceBuilder,
    ):
        pass
