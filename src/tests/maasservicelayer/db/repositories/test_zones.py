...
class TestZonesRepository(RepositoryCommonTests[Zone]):
    ...
    @pytest.fixture
    async def _instance_builder(self) -> ZoneResourceBuilder:
        return (
            ZoneResourceBuilder()
            .with_name("name")
    ...
    async def test_list_with_filters(
        self, repository_instance: ZonesRepository, _created_instance: Zone
    ) -> None:
        ...
        query = QuerySpec(
            where=ZonesClauseFactory.with_ids([1, _created_instance.id])
        )
        zones = await repository_instance.list(None, 20, query)
