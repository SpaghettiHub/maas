...
class TestUsersService:
    ...
    async def test_get_user_ipranges(
        self, users_service: UsersService
    ) -> None:
        users_service.ipranges_service.get_many.return_value = []
        await users_service.get_user_ipranges(user_id=1)
        users_service.ipranges_service.get_many.assert_called_once_with(
            query=QuerySpec(where=IPRangeClauseFactory.with_user_id(1))
        )

    async def test_get_user_staticips(
        self, users_service: UsersService
    ) -> None:
        users_service.staticipaddress_service.get_many.return_value = []
        await users_service.get_user_staticips(user_id=1)
        users_service.staticipaddress_service.get_many.assert_called_once_with(
            query=QuerySpec(where=StaticIPAddressClauseFactory.with_user_id(1))
        )

    async def test_get_user_nodes(self, users_service: UsersService) -> None:
        users_service.nodes_service.get_many.return_value = []
        await users_service.get_user_nodes(user_id=1)
        users_service.nodes_service.get_many.assert_called_once_with(
    ...
    async def test_pre_delete_hook(
        ...
        users_service.ipranges_service.get_many.return_value = resources[
            "ipranges"
        ]
        users_service.nodes_service.get_many.return_value = resources["nodes"]
        if should_raise:
