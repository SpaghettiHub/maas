...
class TestHttpProxyConfig:
    @pytest.mark.parametrize(
        "proxy_url",
        [
            ("http://proxy.example.com"),  # Valid HTTP proxy
            ("https://secure-proxy.com"),  # Valid HTTPS proxy
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("ftp://proxy.example.com"),  # Unsupported protocol
            ("not_a_url"),  # Clearly not a URL
            ("10.0.0.1:5240"),  # Invalid URL format
        ],
    )
...
class TestUpstreamDnsConfig:
    @pytest.mark.parametrize(
        "dns_addresses",
        [
            (["8.8.8.8", "8.8.4.4"]),  # Valid list of IPs
            (
    ...
    def test_valid_upstream_dns(self, dns_addresses):
        ...
        assert len(config_ip_addresses) == len(dns_addresses)
        for dns_address in dns_addresses:
            assert dns_address in config_ip_addresses
...
class TestDNSTrustedAclConfig:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("an-hostname", "an-hostname"),  # Valid hostname
            ("8.8.8.8, 8.8.4.4", "8.8.8.8 8.8.4.4"),  # Valid IPs
            (
                "2001:4860:4860::8888, 2001:4860:4860::8844",
                "2001:4860:4860::8888 2001:4860:4860::8844",
            ),  # Valid IPv6
            ("example.com", "example.com"),  # Valid hostname
            (None, None),  # None should be accepted
        ],
    )
...
class TestNTPServersConfig:
    # Test that validates proper NTP server address formatting
    @pytest.mark.parametrize(
    ...
    def test_validate_value(self, input_value, expected):
        """Test that the validate_value method formats the input correctly."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("999.999.999.999", ValueError),  # Invalid IPv4
            ("ntp..example.com", ValueError),  # Invalid hostname
            ("[2001:db8::g1]", ValueError),  # Invalid IPv6
        ],
    )
    def test_invalid_ntp_servers(self, input_value, expected_exception):
        """Test that invalid IP or hostname raises a ValueError."""
        with pytest.raises(expected_exception):
            NTPServersConfig(value=input_value)

    # Test that the separator works for space/comma mixed delimiters
    @pytest.mark.parametrize(
        "input_value, expected",
        [
            ("192.168.0.1 , ntp.example.com", "192.168.0.1 ntp.example.com"),
    ...
    def test_separator(self, input_value, expected):
        """Test that the separators (spaces and commas) are normalized correctly."""
        config = NTPServersConfig(value=input_value)
        assert config.value == expected

    # Test invalid mixed formats
    @pytest.mark.parametrize(
        "input_value, expected_exception",
        [
...
class TestRemoteSyslogConfig:
    ...
    def test_invalid_values(self, input_value):
        with pytest.raises(ValidationError):
            # Create an instance that will run validation automatically
    ...
    def test_field_initialization(self, input_value, expected):
        # Initialize the RemoteSyslogConfig class with the provided value
...
class TestMAASAutoIPMIKGBmcKeyConfig:
    ...
    def test_invalid_ipmi_key(self, input_value):
        # Test invalid keys that should raise ValueError
