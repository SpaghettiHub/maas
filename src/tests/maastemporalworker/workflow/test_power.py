...
class TestPowerActivity:
    async def test_set_power_state(
        ...
        node = await create_test_machine_entry(fixture, status=NodeStatus.DEPLOYING)
        env = ActivityEnvironment()
        services_cache = CacheForServices()
        power_activity = PowerActivity(db, services_cache, connection=db_connection)
        now = utcnow()
        await env.run(
        ...
        stmt = select(NodeTable.c.power_state, NodeTable.c.power_state_updated).select_from(NodeTable).filter(NodeTable.c.system_id==node["system_id"])
