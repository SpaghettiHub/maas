...
class TestDeployActivity:
    ...
    async def test_get_boot_order_with_netboot(
        ...
        boot_iface = await create_test_interface_entry(
            fixture, node=machine, ips=[ip1]
        )
        other_iface = await create_test_interface_entry(
        ...
        assert boot_order.order == [
            self._stringify_datetime_fields(dev)
...
class TestDeployNWorkflow:
    ...
    async def test_multiple_machine_deploy_success(
        ...
        async with await WorkflowEnvironment.start_time_skipping() as env:
            async with Worker(
                ...
                assert len(calls["set_node_status"]) == 6
                assert len(calls["get_boot_order"]) == 0
                assert len(calls["power_query"]) == 3
                assert len(calls["power_on"]) == 3
                assert len(calls["power_cycle"]) == 3

    async def test_multiple_machine_deploy_all_fail(
        self,
        fixture: Fixture,
        db_connection: AsyncConnection,
        db: Database,
    ...
    async def test_multiple_machine_deploy_one_failure(
        self,
        fixture: Fixture,
        db_connection: AsyncConnection,
        db: Database,
    ...
    async def test_one_ephemeral(
        ...
        async with await WorkflowEnvironment.start_time_skipping() as env:
            async with Worker(
                ...
                assert len(calls["power_query"]) == 3
                assert len(calls["power_on"]) == 3
                assert len(calls["power_cycle"]) == 2
...
class TestDeployWorkflow:
    ...
    async def test_deploy_workflow_timeout(
        ...
        subnet = await create_test_subnet_entry(fixture)
        ip = await create_test_staticipaddress_entry(fixture, subnet=subnet)
        await create_test_interface_entry(fixture, node=machine, ips=[ip])
