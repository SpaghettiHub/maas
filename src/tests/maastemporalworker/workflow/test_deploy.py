...
class TestDeployActivity:
    async def test_set_node_status(
        ...
        node = await create_test_machine_entry(
            fixture, status=NodeStatusEnum.NEW
        )
        ...
        assert status == NodeStatusEnum.READY
    ...
    async def test_deploy_n_workflow_1_node(
        ...
        async with await WorkflowEnvironment.start_time_skipping() as env:
            async with Worker(
                ...
                assert calls["set_node_status"][0] == NodeStatusEnum.DEPLOYED
        ...
        async with await WorkflowEnvironment.start_time_skipping() as env:
            async with Worker(
                ...
                assert calls["set_node_status"] == [
                    NodeStatusEnum.DEPLOYED for _ in range(3)
                ]
                assert len(calls["get_boot_order"]) == 0
