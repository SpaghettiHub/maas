...
class TestHttpBakeryAsyncClient:
    ...
    async def test_discharge_with_interaction_required_error(
        self, new_bakery, third_party_url, monkeypatch
    ):
        d = _DischargerLocator(third_party_url)
        b = new_bakery("loc", d)
        ...
        async def monkey_patch_discharge_handler(self, request):
            ...
            return web.json_response(resp, status=401, headers=self.headers)
        ...
        class MyInteractor(httpbakery.LegacyInteractor):
            async def legacy_interact(self, client, location, visit_url):
                raise httpbakery.InteractionError("cannot visit")

            async def interact(self, ctx, location, interaction_required_err):
                pass

            def kind(self):
                return httpbakery.WEB_BROWSER_INTERACTION_KIND

        client = HttpBakeryAsyncClient(interaction_methods=[MyInteractor()])
        url = local_server.name
        with pytest.raises(httpbakery.InteractionError) as exc_info:
            await client.request(method="GET", url=url)

        assert (
            exc_info.value.args[0]
            == "cannot start interactive session: cannot visit"
        )
