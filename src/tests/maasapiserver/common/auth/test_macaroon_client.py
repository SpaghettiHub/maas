...
from maasapiserver.common.auth.models import Resource
...
class TestRbacAsyncClient:
    ...
    async def test_allowed_for_user_all_resources(
        ...
        mock_aioresponse.get(
            f"{RBAC_BASE_URL}/resources/maas/allowed-for-user?u=user&p=admin",
            payload=expected_response,
        )

        resp = await rbac_client.allowed_for_user("maas", "user", ["admin"])
        ...
        mock_aioresponse.assert_called_once_with(
            method="GET",
            url=f"{RBAC_BASE_URL}/resources/maas/allowed-for-user",
