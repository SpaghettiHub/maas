...
class TestPermissionsFunctions:
    ...
    async def test_get_user(self, auth_client: AsyncClient) -> None:
        user_response = await auth_client.post(
            f"{V3_API_PREFIX}/user", json=self._build_request("test", {UserRole.USER})
        )
        ...
        assert user_response.status_code == 200
        user_response = await auth_client.post(f"{V3_API_PREFIX}/user_permissions")
        ...
        assert user_response.status_code == 403
        user_response = await auth_client.post(f"{V3_API_PREFIX}/admin_permissions")
        assert user_response.status_code == 401
        user_response = await auth_client.post(
            f"{V3_API_PREFIX}/admin_permissions",
            json=self._build_request("test", {UserRole.ADMIN}),
