...
class TestMachinesService:
    ...
    async def test_get_usb_devices(
        ...
        machines_repository_mock = Mock(MachinesRepository)
        machines_repository_mock.get_usb_devices = AsyncMock(
        ...
        usb_devices_list = await machines_service.get_usb_devices(
            system_id="dummy", token=None, size=1
        )
        machines_repository_mock.get_usb_devices.assert_called_once_with(
            system_id="dummy", token=None, size=1
        )
