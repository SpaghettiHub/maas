...
class TestInterfaceRepository:
    ...
    async def test_list_successfull_but_ip_address_empty_if_only_dhcp_links(
        self, db_connection: AsyncConnection, fixture: Fixture
    ) -> None:
        # A dhcp link gets it ip address from discovered links on the same subnet
        # if we don't have any, this will be None.
        ...
        def _assert_interface_in_list(
            ...
            # check the links match with expected None ip address
            for created_link, response_link in zip(
                sorted(interface.links, key=lambda link: link.id),
                sorted(interface_response.links, key=lambda link: link.id),
            ):
                assert created_link.id == response_link.id
                assert created_link.ip_type == response_link.ip_type
                # we create with an ip, but don't return it from the DB
                assert created_link.ip_address is not None
                assert response_link.ip_address is None
    ...
    async def test_list_interfaces_use_discovered_ip_for_dhcp_links(
        ...
        def _assert_interface_in_list(
            ...
            # check they match, excluding the discovered ip
            assert len(interface.links) == 1 + len(interface_response.links)
            assert sorted(
                interface.links,
        ...
        for i in range(0, interface_count):
            ...
            # update all of the dhcp addresses with the discovered address
            for link in this_interface.links:
                link.ip_address = discovered_ip["ip"]
