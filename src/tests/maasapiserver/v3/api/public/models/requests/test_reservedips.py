...
class TestReservedIPCreateRequest:
    async def test_ip_already_in_use(
        ...
        with pytest.raises(ValidationException) as e:
            await ReservedIPCreateRequest(
                ip=ip, mac_address="00:00:00:00:00:00"
            ).to_builder(services_mock)
    ...
    async def test_ip_inside_dynamic_range(
        ...
        with pytest.raises(ValidationException) as e:
            await ReservedIPCreateRequest(
                ip=ip, mac_address="00:00:00:00:00:00", subnet_id=1
            ).to_builder(services_mock)
    ...
    async def test_no_suitable_subnet_for_ip(
        self, services_mock: ServiceCollectionV3
    ) -> None:
        ip = IPv4Address("10.10.0.3")
        services_mock.staticipaddress = Mock(StaticIPAddressService)
        ...
        services_mock.subnets.find_best_subnet_for_ip.return_value = None
        with pytest.raises(ValidationException) as e:
            await ReservedIPCreateRequest(
                ip=ip, mac_address="00:00:00:00:00:00"
            ).to_builder(services_mock)
        ...
        assert e.value.details[0] == BaseExceptionDetail(
            ...
            message=f"Could not find a suitable subnet for {ip}. Please create the subnet first.",
            field=None,
            location=None,
        )
        services_mock.staticipaddress.get_one.assert_called_once_with(
        ...
        services_mock.subnets.find_best_subnet_for_ip.assert_called_once_with(
            ip
        )
    ...
    async def test_ip_not_in_subnet(
        ...
        services_mock.subnets = Mock(SubnetsService)
        services_mock.subnets.find_best_subnet_for_ip.return_value = (
            subnet_mock
        )
        with pytest.raises(ValidationException) as e:
            await ReservedIPCreateRequest(
