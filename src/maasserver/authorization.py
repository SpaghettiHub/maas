...
def can_view_boot_entities(user: User) -> bool:
    from maasserver.rbac import rbac

    if rbac.is_enabled():
        return user.is_superuser
    return get_openfga_client().can_view_boot_entities(user)


def can_edit_boot_entities(user: User) -> bool:
    from maasserver.rbac import rbac

    if rbac.is_enabled():
        return user.is_superuser
    return get_openfga_client().can_edit_boot_entities(user)
...
def can_view_license_keys(user: User):
    from maasserver.rbac import rbac

    if rbac.is_enabled():
        return user.is_superuser
    return get_openfga_client().can_view_license_keys(user)


def can_edit_license_keys(user: User):
    from maasserver.rbac import rbac

    if rbac.is_enabled():
        return user.is_superuser
    return get_openfga_client().can_edit_license_keys(user)
