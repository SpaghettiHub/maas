...
class TemporalService(Service):
    def __init__(self, connection: AsyncConnection, temporal: Client):
    ...
    async def post_commit(self) -> None:
        ...
        for _, arguments in self._post_commit_workflows.items():
    ...
    def register_or_update_workflow_call(
        ...
        if self.workflow_is_registered(workflow_name, workflow_id=workflow_id):
            if (
                not override_previous_parameters
                and not parameter_merge_function
            ):
                raise ValueError(
            ...
            if not override_previous_parameters:
                parameter = parameter_merge_function(
