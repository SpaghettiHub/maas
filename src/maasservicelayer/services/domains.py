...
class DomainsService(BaseService[Domain, DomainsRepository, DomainBuilder]):
    ...
    async def render_json_for_related_rrdata(
        ...
        result = await self.v3_render_json_for_related_rrdata(
            domain_id, user_id, include_dnsdata, as_dict, with_node_id
        )
        if isinstance(result, dict):
            return OrderedDict(
                {
                    key: [v.to_dict(with_node_id=with_node_id) for v in values]
                    for key, values in result.items()
                }
            )
        else:
            return [v.to_dict(with_node_id=with_node_id) for v in result]
