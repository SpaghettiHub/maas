...
class UsersService(BaseService[User, UsersRepository, UserBuilder]):
    ...
    async def get_user_ipranges(self, user_id: int) -> list[IPRange]:
        return await self.ipranges_service.get_many(
            query=QuerySpec(where=IPRangeClauseFactory.with_user_id(user_id))
        )

    async def get_user_staticips(self, user_id: int) -> list[StaticIPAddress]:
        return await self.staticipaddress_service.get_many(
            query=QuerySpec(
    ...
    async def get_user_nodes(self, user_id: int) -> list[Node]:
        return await self.nodes_service.get_many(
    ...
    async def pre_delete_hook(self, resource_to_be_deleted: User) -> None:
        owned_ipranges = await self.get_user_ipranges(
            resource_to_be_deleted.id
        )
        owned_staticips = await self.get_user_staticips(
            resource_to_be_deleted.id
        )
        owned_nodes = await self.get_user_nodes(resource_to_be_deleted.id)
