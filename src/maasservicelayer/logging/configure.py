...
def reorder_keys(_, __, event_dict):
    # Reorder keys to place 'context_id' and 'event' at the beginning
    reordered_dict = {
        key: event_dict[key]
        for key in ["context_id", "event"]
        if key in event_dict
    }
    # Add the rest of the keys after
    reordered_dict.update(
        {k: v for k, v in event_dict.items() if k not in reordered_dict}
    )
